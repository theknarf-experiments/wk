 ...
 *
 * 1. The mount table (<mntent.h>): a wk node's filesystem is a private
 *    in-memory vfs with no mounts to enumerate, so the table is empty.
...
/* ---- mount table: always empty ---- */

FILE *setmntent(const char *filename, const char *type) {
  (void)filename;
  (void)type;
  /* Non-NULL so callers proceed; getmntent immediately reports end-of-table. */
  return (FILE *)&errno;
}
