 ...
 * filesystem (mount it at /bin/coreutils.wasm, e.g. by wiring a BindMount or
 * building an image that COPYs it), a couple of tools in sequence, and then a
 * two-stage pipeline by feeding the first program's stdout into the second's
 * stdin — which is how a shell would do it with this capability.
 */
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    /* 1. A plain command. */
    const char *echo_args[] = {"--coreutils-prog=echo", "hello", "from", "an", "exec'd", "program", NULL};
     ...
     *    files as its parent. */
    const char *ls_args[] = {"--coreutils-prog=ls", "-1", "/", NULL};
    ...
    const char *seq_args[] = {"--coreutils-prog=seq", "1", "5", NULL};
    ...
    if (!r.error) {
        ...
        wk_result r2;
        const char *wc_args[] = {"--coreutils-prog=wc", "-l", NULL};
    ...
    const char *mkdir_args[] = {"--coreutils-prog=mkdir", "-p", "/made-by-child", NULL};
