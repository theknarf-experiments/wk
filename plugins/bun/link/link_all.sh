...
"$WASI_SDK/bin/clang++" --target=wasm32-wasip2 -fno-exceptions -O2 \
  ...
  $S/bunobj/*.o /tmp/gen_*.o /tmp/mod_*.o /tmp/wasi_stubs.o /tmp/quic_stubs.o /tmp/trap_stubs.o /tmp/trap_stubs_cxx.o /tmp/trap_stubs_v8.o /tmp/imrc.o /tmp/hdr_*.o /tmp/vlib/libzstd.a /tmp/vlib/libbrotli.a /tmp/vlib/libbssl_crypto.a /tmp/vlib/libusockets.a /tmp/libuwsockets.o /tmp/us_root_certs.o /tmp/vlib/libcares.a /tmp/vlib/libarchive.a /tmp/vlib/libz.a /tmp/vlib/libdeflate.a /tmp/vlib/libsqlite3.a /tmp/vlib/libllhttp.a /tmp/vlib/libspng.a /tmp/vlib/libturbojpeg.a /tmp/bun_simdutf.o /tmp/mainobj/bun_main.o /tmp/main_shim.o \
  ...
  -Wl,-z,stack-size=8388608 -Wl,--error-limit=0 -Wl,--export=cabi_realloc -Wl,--export=main \
