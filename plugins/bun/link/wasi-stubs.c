...
// Real signature returns void (upstream lshpack.h); the old `int` return made
// bun's `(struct lshpack_dec*) -> void` call a signature_mismatch trap that
// crashed the runtime on any node:http2 use. Non-functional still (no vendored
// lshpack build), but now http2 fails gracefully instead of trapping.
void lshpack_dec_init(struct lshpack_dec* d) { (void)d; }
