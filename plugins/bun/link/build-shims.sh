...
# into the /tmp/*.o objects that link_all.sh consumes. This is the single
...
for s in "${plain[@]}"; do
    "$CC" --target=wasm32-wasip2 -O2 -c "$s.c" -o "/tmp/$s.o"
done
"$CC" --target=wasm32-wasip2 -O2 -c quic_syscall_stubs.c -o /tmp/quic_stubs.o
"$CC" --target=wasm32-wasip2 -O2 -c wasi-stubs.c        -o /tmp/wasi_stubs.o
# epoll_impl pulls in the wasi-compat <sys/epoll.h> shim (wasi-libc has none).
"$CC" --target=wasm32-wasip2 -O2 -I ../wasi-compat -c epoll_impl.c -o /tmp/epoll_impl.o
# main_shim pulls in <mimalloc.h> to disable purging (mi_option_set*).
"$CC" --target=wasm32-wasip2 -O2 -I "$MI_INC" -c main_shim.c -o /tmp/main_shim.o

# Vendored C deps compiled for the same link (own scripts, own fetch/flags).
WASI_SDK="$WASI_SDK" bash ./build_exec_picohttp.sh   # picohttpparser, exec_host, wkexec, pipe
WASI_SDK="$WASI_SDK" bash ./build_lshpack.sh          # lshpack, xxhash

echo "built all link shims + vendored C deps into /tmp"
